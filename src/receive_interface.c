...
static int handler(void *user, const char *section, const char *name,
                   const char *value)
{
    struct gdevice *pdev = NULL; // 临时指针 链表头
    // printf("section:%s, name:%s, value:%s\n..............********..........\n", section, name, value);

    if (NULL == pdevhead)
    {
        pdevhead = (struct gdevice *)malloc(sizeof(struct gdevice)); // 申请空间
        memset(pdevhead, 0, sizeof(struct gdevice));                 // 初始化空间
        strcpy(pdevhead->dev_name, section);                         // 外设名称
        pdevhead->next = NULL;                                       // 第一个添加的设备最后会是最后一个节点指向NULL
    }
    else if (0 != strcmp(section, pdevhead->dev_name))
    {
        pdev = (struct gdevice *)malloc(sizeof(struct gdevice)); // 申请空间
        memset(pdev, 0, sizeof(struct gdevice));                 // 初始化空间
        strcpy(pdev->dev_name, section);                         // 外设名称
        pdev->next = pdevhead;
        pdevhead = pdev;
    }
    ...
    if (NULL != pdevhead)
    {
        if (MATCH(pdevhead->dev_name, "key")) // 设备节点标识
        {
            sscanf(value, "%x", &pdevhead->key); // 字符串转16进制并存入pdevhead->key中
            printf("pdevhead->key:%x\n", pdevhead->key);
        }
        else if (MATCH(pdevhead->dev_name, "gpio_pin")) // 引用针脚
        {
            pdevhead->gpio_pin = atoi(value); // 字符转数值型
        }
        else if (MATCH(pdevhead->dev_name, "gpio_mode")) // 针脚输入输出模式
        {
            // printf("pdevhead->gpio_mode-value:%s,len:%ld,strlen:%ld\n", value, sizeof(value), strlen(value));
            // int a = strcmp(value, "OUTPUT");
            // printf("a:%d\n", a);
            // printf("%s|%s|%d|pdevhead->gpio_mode-value:%s\n", __FILE__, __func__, __LINE__, value);
            if (0 == strcmp(value, "OUTPUT"))
            {
                // printf("%s|%s|%d|pdevhead->gpio_mode-value:%s\n", __FILE__, __func__, __LINE__, value);

                pdevhead->gpio_mode = OUTPUT;
            }
            else if (0 == strcmp(value, "INPUT"))
            {
                // printf("%s|%s|%d|pdevhead->gpio_mode-value:%s\n", __FILE__, __func__, __LINE__, value);
        ...
        else if (MATCH(pdevhead->dev_name, "gpio_status")) // 针脚电平的
        {
            // printf("%s|%s|%d|pdevhead->gpio_status:%s\n", __FILE__, __func__, __LINE__, value);

            if (0 == strcmp(value, "LOW"))
            {
        ...
        else if (MATCH(pdevhead->dev_name, "check_face_status")) // 是否启用语音模块
        {
            pdevhead->check_face_status = atoi(value);
        }
        else if (MATCH(pdevhead->dev_name, "voice_set_status")) // 是否启用摄像头
        {
...
{
    if (ini_parse("./peripheral.ini", handler, NULL) < 0)
    {
        printf("Can't load 'peripheral.ini'\n");
        return 1;
    }
